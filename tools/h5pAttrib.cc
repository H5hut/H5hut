...
static void print_all(H5PartFile* file)
{
   ...
   if(print_header)
   {
      ...
      for(i=0; i<num_attrib; i++)
      {
         ...
         fprintf(stdout, "\tAttribute #%d = %s\n",i,file_attrib);
         fprintf(stdout, "\tThere are %d elements in the attribute\n",num_elem);
   ...
   else
   {
      ...
      for(i=0; i<num_attrib; i++)
      {
         ...
         if (type == H5T_NATIVE_INT64)
         {
            value = (long long int*)malloc(sizeof(long long int)*num_elem);
            H5PartReadFileAttrib(file, file_attrib, (long long int*)value);

            fprintf(stdout, "\tAttribute Type: H5T_NATIVE_INT64\n");

            fprintf(stdout, "\tPrinting %d element value(s):\n", num_elem);
            ...
            {
               fprintf(stdout, "%ld  ",((long long int*)value)[j]);
               k++;
               if(k==5)
               {
                  fprintf(stdout, "\n");
         ...
         else if (type == H5T_NATIVE_CHAR)
         {
            ...
            fprintf(stdout, "\tPrinting string of length %d:\n", num_elem);
   ...
   for(timestep_ctr=0; timestep_ctr<nt; timestep_ctr++)
   {
       ...
       if(print_header)
       {
          for(i=0; i<num_attrib; i++)
          {
             ...
             fprintf(stdout, "\tAttribute #%d = %s\n",i,step_attrib);
             fprintf(stdout, "\tNumber of elements in the attribute: %d\n",num_elem);
       ...
       else
       {
          for(i=0; i<num_attrib; i++)
          {
             ...
             {
                value = (long long int*)malloc(sizeof(long long int)*num_elem);
                H5PartReadStepAttrib(file, step_attrib, (long long int*)value);
      ...
      if(print_header)
      {
         for(i=0; i<num_dataset; i++)
         {
            ...
            fprintf(stdout, "\tNumber of elements in the dataset: %ld\n",nparticles);
      ...
      else
      {
         for(i=0; i<num_dataset; i++)
         {
            ...
            if (type == H5T_NATIVE_INT64)
            {
               value = (long long int*)malloc(sizeof(long long int)*nparticles);
               H5PartReadDataInt64(file, data_name, (long long int*)value);
               fprintf(stdout, "\tDataset Type: H5T_NATIVE_INT64\n");
               fprintf(stdout, "\tPrinting %ld element value(s):\n", nparticles);
...
static void print_file_attributes(H5PartFile* file, char * garbage)
{
    ...
    if(print_header)
    {
       ...
       fprintf(stdout, "\n");
       num_attrib = H5PartGetNumFileAttribs(file);
       fprintf(stdout, "The number of file attributes for file %s is %d ...\n", global_fname, num_attrib);
...
static void print_step_attributes(H5PartFile* file, char *attr)
{
    ...
    if(print_header)
    {
       ...
       num_attrib = H5PartGetNumStepAttribs(file);
       fprintf(stdout, "The number of step attributes for timestep #%d is %d ...\n\n", atoi(attr), num_attrib);
       for(i=0; i<num_attrib; i++)
       {
          H5PartGetStepAttribInfo(file, i, step_attrib, MAX_LEN, &type, &num_elem);
          fprintf(stdout, "Attribute #%d = %s\n",i,step_attrib);
