...
c        Now start writing the data arrays for this step
         ...
         call h5pt_writedata_r8(file,"y",Y)
         call h5pt_writedata_r8(file,"z",Z)
         call h5pt_writedata_r8(file,"px",PX)
         call h5pt_writedata_r8(file,"py",PY)
         call h5pt_writedata_r8(file,"pz",PZ)
         call h5pt_writedata_i8(file,"id",ID)
         do J=1,npoints
            ID(J)=ID(J)+10
         enddo
...
c *****************Now Reopen for Reading ******************
      ...
      call h5pt_setview(file,pointoffset,pointoffset+npoints)
      print *,"  Pointoffset=",pointoffset,"  Local NP=",npoints
c Now we need to set our view to read correct points
...
c         call h5prt_readdata(file,step,X,Y,Z,PX,PY,PZ,ID)
         do J=1,10
            print *,"    ID(",J,")==",ID(J)
         enddo
      enddo
